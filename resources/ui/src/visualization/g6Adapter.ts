 ...
 * G6 数据适配器
 *
 * - toG6Data(): AdapterNode/AdapterEdge → G6 NodeData/EdgeData 格式转换
 * - mergeG6Data(): 增量合并，按 id 去重节点，按 source|target 去重边
 * - deduplicateAndLimitEdges(): 边去重 + 总数上限截断
 */
...
/** 同 (source,target) 仅保留一条边，总数上限避免渲染卡顿 */
 ...
 *
 * @param edges - 原始边列表
 * @returns { edges: 去重截断后的边, truncated: 是否发生截断, originalCount: 原始数量 }
 */
export function deduplicateAndLimitEdges(edges: EdgeData[]): {
  edges: EdgeData[];
  truncated: boolean;
  ...
  return {
    edges: out,
    truncated: originalCount > out.length,
    originalCount,
  };
}
