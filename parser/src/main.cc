...
void PrintUsage(const char* prog) {
  std::cerr
      ...
      << "  --type <call_graph|class_graph|data_flow|control_flow>\n"
...
int main(int argc, char* argv[]) {
  ...
  if (sub == codexray::Subcommand::kParse) {
    parse_opts.progress_stdout = [](size_t done, size_t total) {
      ...
      std::cout << "{\"progress\":" << pct << ",\"total\":" << total << "}\n" << std::flush;
    };
    if (!codexray::RunParse(parse_opts)) {
      std::cerr << "Parse failed.\n";
      return static_cast<int>(codexray::ExitCode::kDbWriteFailed);
    }
    return 0;
  }
  ...
  if (sub == codexray::Subcommand::kQuery) {
    ...
    std::string json;
    if (query_opts.query_type == "call_graph")
