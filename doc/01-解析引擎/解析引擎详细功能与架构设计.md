...
- **能力**：并行解析（默认并行度 `max(1, 系统核心数 - 2)`）、懒解析（优先解析目录 + 查询时按需解析剩余）、优先解析目录配置、**增量更新**（仅变更 TU 重解析并合并回 DB）、**历史解析记录**（parse_run 表与 list-runs 接口）。
...
| 并行与懒解析调度 | 线程池、TU 队列、默认 parallel=max(1,cores-2)；懒解析下首轮只跑优先 TU，查询时触发剩余 TU | scheduler/ | 400-600 | common | 2.1 --parallel, --lazy, --priority-dirs | 1.4, 1.5 |
...
| 公共（日志/错误码/工具） | 日志落盘、错误码常量、USR/路径辅助 | common/ | 200-400 | - | 接口约定 §4 错误码 | - |
...
5. driver 将 TU 列表（全量或仅优先/仅变更）提交给 scheduler；scheduler 并行执行每个 TU：运行 call_graph、class_relation、data_flow、control_flow 四个 FrontendAction，结果汇总。
