...
- **关键实现要点**：遍历**所有** FunctionDecl（含仅声明与定义）；USR 使用 index::generateUSRForDecl；**定义位置** def_* 与**声明位置** decl_* 均落库（symbol 表含 decl_file_id/decl_line/decl_column/decl_line_end/decl_column_end，迁移增加）；kind 区分 function/method/constructor/destructor（CXXMethodDecl/CXXConstructorDecl/CXXDestructorDecl）；def_in_tu_file/decl_in_tu_file 标记是否在当前 TU 主文件，driver 按 USR 合并多 TU 结果后填 def_file_id/decl_file_id；CallExpr 处理同前（direct / via_function_pointer）。
...
- **功能**：按 USR、文件、符号名、类型等查询 symbol、call_edge、class、class_relation、global_var、data_flow_edge、cfg；支持「深度」限制（如默认 3 层）；支持「从某节点向前/向后扩展」；**支持按文件+行号查询符号**（def 或 decl 位于该行）。不输出 JSON，仅返回结构化数据供 query 层使用。
- **公开接口**：`QuerySymbolByUsr`；`QuerySymbolsByFile`；`QuerySymbolsByFileAndLine(db, file_id, line, column_hint)`（按 def/decl 位置查）；`QueryCallEdges`；`QueryCallGraphExpand`；类、数据流、CFG 类似。
...
- **功能**：实现 call_graph、class_graph、data_flow、control_flow 及 **symbol_at**（按文件+行号返回符号列表）的 JSON 输出。节点含 id、usr、name、kind、**definition**（file、line、column）、**definition_range**、**declaration** 与 **declaration_range**（若有）；边含 caller、callee、call_site、edge_type 等。CLI 支持 `--file` + `--line` [--column]：若未给 `--symbol` 则先按位置解析出符号再查；`--type symbol_at` 时直接返回该位置符号数组。
- **公开接口**：`QueryCallGraphJson`、`QueryClassGraphJson`、`QueryDataFlowJson`、`QueryControlFlowJson`；`QuerySymbolAtLocationJson(db, project_id, file_path, line, column)`。
- **关键实现要点**：file/line 从 DB 的 file_id 解析为路径（file 表）；definition/declaration 与 symbol 表 def_*/decl_* 对应；主仓库默认 depth=3。
- **预估行数**：450-650。
...
- **现象**：所有 TU 解析完（进度到 100%）后进程仍要等待较长时间才退出，实为批量写 DB 阶段。
- **进度**：将「写入数据库」算作一步，总步数 = TU 数 + 1；解析完成时进度约为 98%（未到 100%），DB 写完后才上报 100%，避免长时间停在 100% 无反馈。
- **DB 加速**：连接打开后设置 `PRAGMA journal_mode=WAL`、`PRAGMA synchronous=NORMAL`、`PRAGMA cache_size=-64000`（db/connection.cc）；WriteCallEdges 对 USR→symbol_id 做内存缓存，避免同一外部符号重复 SELECT/INSERT；写入阶段复用解析时已得到的 file_id，不再对每个 TU 重复 EnsureFile。
...
**解析速率优化（不改变并行度）：** Include 参数缓存（`GetClangIncludeEnvExtraArgs()` 进程内缓存）；`GetClangIncludeEnv` 返回 `const ClangIncludeEnv&` 避免拷贝；热路径去掉每 TU/每进度的 LogInfo；Logger 仅对 ERROR 做 fflush。
