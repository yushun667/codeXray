...
- **公开 API**：`getWorkspaceRoot(): Uri | undefined`、`getDatabasePath(): string`、`getCompileCommandsPath(): string`、`getParserPath(): string`、`getParserOptions(): ParserOptions`、`getAgentEndpoint(): string`、**`getLlmConfig(): LlmConfig`**（大模型配置：provider、model、endpoint、apiKey，供启动 Agent 时写入环境或配置文件）、`getLogLevel(): string`、`getLogPath(): string | undefined`；可选 `init(context)` 注入 context。
...
- **功能**：根据 getAgentEndpoint() 建立与 Agent 的连接（stdio 子进程 / Unix socket / HTTP）；**启动 Agent 时根据 getLlmConfig() 将大模型配置写入配置文件或环境变量**，使 Agent 支持设置大模型；发送 chat 请求（含 session_id、message、context：file/symbol/selection；可选 llm_override）；接收流式或一次性回复并回调；维护 session_id。不负责聊天 UI 渲染。
...
| 模块 | 主要接口 |
|------|----------|
| Config | getWorkspaceRoot(), getDatabasePath(), getCompileCommandsPath(), getParserPath(), getParserOptions(), getAgentEndpoint(), **getLlmConfig()**, getLogLevel(), getLogPath() |
...
- **配置**：codexray.compileCommandsPath, codexray.databasePath, codexray.parserParallelism, codexray.parserLazy, codexray.parserPriorityDirs, codexray.parserIncremental, codexray.agentEndpoint, **codexray.llmProvider, codexray.llmModel, codexray.llmEndpoint, codexray.llmApiKey**（必须支持设置大模型）, codexray.logLevel, codexray.logPath。
