...
- **布局**：采用**双向树状布局**（`graphLayout.ts`）：以无入边或入边最少的节点为根，前驱在左、后继在右（rankdir 为左右方向），同层节点**同一 x 坐标、y 等距并整体居中对齐**；根节点居中。**节点、边与边、边与节点不重叠**：层间距 RANK_SEP ≥ 节点宽度 + 余量、同层 NODE_SEP ≥ 节点高度 + 余量。**边**：使用 **smoothstep 折线**，折弯处通过 **pathOptions.borderRadius 圆弧**过渡（GraphCore 中 defaultEdgeOptions）。**边数量**：同 (source,target) 仅保留一条、总数上限 2500 条，避免数万条边导致卡顿；超出时顶部提示「边数量较多，已去重并仅展示前 2500 条」。
- **节点**：自定义 **GraphNode**（`GraphNode.tsx`），minWidth 200、maxWidth 360，`title={label}` 悬停显示完整名称，避免名称显示不全。
- **图交互能力（必须支持）**：
