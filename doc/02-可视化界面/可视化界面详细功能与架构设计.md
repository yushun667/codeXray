...
- **选定**：**React Flow**（[reactflow](https://github.com/xyflow/xyflow)，npm: `reactflow`），为 React 生态内成熟的节点+边可视化库。
- **理由**：以 React 组件与 Hooks 方式使用，与前端框架统一；支持节点、边、缩放/平移、节点点击回调（`onNodeClick`）、自定义节点/边；维护活跃、文档完善，适合调用链/类关系/数据流/控制流四种视图共用同一图核心组件，通过 **nodes/edges 数据与布局算法** 区分视图，符合「图核心 + 各视图适配层」的 500-800 行划分。
- **适配方式**：图核心（`visualization/GraphCore.tsx` 或 `graph.tsx`）使用 `<ReactFlow>` 组件，接收 type + 标准 JSON，内部将数据转为 React Flow 的 `nodes`/`edges`；各视图（call-graph/class-graph/data-flow/control-flow）负责将 [01-解析引擎/接口约定](../01-解析引擎/接口约定.md) 第 3 节的查询结果转换为 `nodes`/`edges`，并应用布局。节点点击时从 node.data 读取 definition/definition_range，通过 postMessage 发送 gotoSymbol。
- **布局**：层次布局采用 **@dagrejs/dagre**（npm: `@dagrejs/dagre`）计算节点位置后写入 `nodes` 的 `position`，再交给 React Flow 渲染；其他视图可使用 React Flow 内置的布局或简单拓扑排序 + 坐标计算。React Flow 官方文档提供与 dagre 集成的示例。
- **图交互能力（必须支持）**：
  - **节点**：支持**选中**（单击选中、多选）、**框选**（拖拽矩形框多选）、**拖拽移动**（节点位置可调）、**删除**（选中后 Delete/Backspace 或工具栏删除，删除后同步更新 nodes/edges 状态）；支持**折叠/展开**（如按子节点或邻接边显隐，由 node.data 或本地 state 控制）。
  - **画布**：支持**放大/缩小**（滚轮或缩放控件）、**平移**（拖拽空白区域移动视图）；由 React Flow 的 `panOnDrag`、`zoomOnScroll`、`zoomOnPinch`、`panOnScroll` 等配置与默认控件实现。
  - **图查询与扩展**：**默认仅查询并展示 3 层图**（深度由主仓库在发起查询时指定，如 depth=3）；**右键节点**提供「继续查询前置节点」「继续查询后置节点」菜单项，UI 发送 `queryPredecessors`/`querySuccessors`（携带当前 type、节点 id 或符号信息），主仓库从 DB 查询增量数据并返回；UI 将返回的 nodes/edges **合并到当前图界面**（不新开标签），去重后追加并可选局部或全图重布局。
- **数据格式**：图核心只消费上述接口约定中的 JSON（节点含 usr、definition、definition_range；边含 edge_type、call_site 等），不解析 DB。
...
| **图布局与合并** | dagre 布局（nodes+edges→带 position 的 nodes）；graphAppend 按 id 去重合并 | visualization/graphLayout.ts, graphMerge.ts | 200-400 | shared | 无（图核心内部使用） | - |
...
- **图/树可视化**：原「图核心」同时承担画布、布局、合并、右键菜单、折叠展开，实现易达 1000+ 行，**已按上表拆分为三块**：① 图核心画布（ReactFlow + 状态 + 事件，调用 layout/merge）；② 图布局与合并（纯逻辑，dagre + 去重合并）；③ 图右键菜单与折叠（UI + 状态 hook）。拆分后单文件/单职责均在 500-800 或 200-450 行内。
...
- **功能**：React Flow 画布容器；维护 nodes/edges 状态；接收主仓库传入的 type 与 data（或由上层适配器转换后的 nodes/edges），**默认展示深度 3 层**由主仓库查询时保证。**必须支持**：节点选中/框选/拖拽移动/删除、画布放大/缩小/平移（React Flow 配置）；节点单击时从 node.data 取 definition/definition_range，postMessage(gotoSymbol)。收到 `graphAppend` 时调用 **graphMerge** 去重合并，再调用 **graphLayout** 重算位置后更新 state。不包含右键菜单与折叠逻辑，由 5.5、5.6 提供。
...
- **关键实现要点**：布局与合并为纯函数或纯逻辑，无 UI；dagre 选项（方向、rankSep 等）可与 graphType 挂钩；merge 时边也需去重（如按 source+target+edgeType）。
- **预估行数**：200-400（两文件合计）。
...
1. 主仓库打开可视化 Webview，传入 type 与 data（**默认查询深度 3 层**，如通过查询参数 depth=3 或主仓库调用解析引擎时指定）。
...
- **主仓库职责**：创建并持有 Webview、加载 UI 资源（HTML/JS/CSS）、收发 postMessage、解析 action 并调用 ParserService/AgentService/GotoSymbol；**首次打开图时按默认深度（如 3 层）查询并传入 data**；收到 `queryPredecessors`/`querySuccessors` 时从 DB 查询增量图数据并以 `graphAppend` 回传；将解析结果、历史、进度、Agent 回复等通过 postMessage 传给 UI；注入 Codicon 资源与主题变量（若采用 2.3、2.4 约定）。
