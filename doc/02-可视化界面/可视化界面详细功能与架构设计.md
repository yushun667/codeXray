...
- **布局**：采用 **Dagre**（`@dagrejs/dagre`，`dagreLayout.ts`）有向图分层布局，方向 LR（左→右），**节点碰撞检测**由 Dagre 保证不重叠；支持每节点 `width`/`height`（由 CustomNode 测量回传）。**边**：**smoothstep** 折线 + **pathOptions.borderRadius** 圆角 + **animated: true** 区分调用关系。**边数量**：同 (source,target) 去重、总数上限 2500 条；超出时顶部提示。
- **节点**：自定义 **CustomNode**（`CustomNode.tsx`，type: `customNode`）：**函数名多行居中**（自动换行）、**宽高根据文字测量**（隐藏 div 测量后经 `onDimensions` 回传，再跑 Dagre 重排）、**自定义样式**（`data.style`：backgroundColor、color、borderColor、borderRadius）。示例：`{ id: '1', type: 'customNode', data: { label: '函数A\n多行' }, position: { x: 0, y: 0 } }`。
- **导出**：host 发送 `action: 'exportGraph'`，图 Webview 回复 `action: 'graphExported'`、`payload: GraphExportPayload`（nodes/edges 可 JSON 序列化，用于动态渲染）。
- **图交互能力（必须支持）**：
  ...
  - **图查询与扩展**：**默认查询深度为 2 层**，可通过配置 **CodeXray: Query Depth**（`codexray.queryDepth`，1–10）修改；**右键节点**提供「继续查询前置节点」「继续查询后置节点」菜单项，UI 发送 `queryPredecessors`/`querySuccessors`，主仓库从 DB 查询增量数据并返回；UI 将返回的 nodes/edges **合并到当前图界面**（不新开标签），去重后全图重布局。
