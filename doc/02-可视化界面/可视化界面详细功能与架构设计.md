...
- **布局**：采用**双向树状布局**（`graphLayout.ts`）：以无入边或入边最少的节点为根，前驱在左、后继在右（rankdir 为左右方向），同层节点**同一 x 坐标、y 等距并整体居中对齐**；根节点居中。**节点、边与边、边与节点不重叠**：层间距 RANK_SEP ≥ 节点宽度 + 余量、同层 NODE_SEP ≥ 节点高度 + 余量。**边**：使用 **smoothstep 折线**，折弯处通过 **pathOptions.borderRadius 圆弧**过渡（GraphCore 中 defaultEdgeOptions）。
- **图交互能力（必须支持）**：
